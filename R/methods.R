...
plot_apply_theme <- function(p, cex.legend, legend.pos, cex.lab, cex.axis,
                             cex.main, angle, x.h, x.v) {
    p + theme(
...
plot_histogram_layer <- function(p, data, hist.adjust, rect.min, rect.length,
                                 Ntr, max.count.pos, cex.text, type_col = NULL) {
    ...
    xx <- range(data$time)
    rect.data <- if (!is.null(type_col)) data[which(data[[type_col]] == "tr"), ] else data
...
plot.tjbal <- function(x,
    ...
    if (is.null(cex.main) == FALSE) {
        if (is.numeric(cex.main) == FALSE) stop("\"cex.main\" is not numeric.")
        cex.main <- 18 * cex.main
    } else {
        cex.main <- 18
    }
    if (is.null(cex.lab) == FALSE) {
        if (is.numeric(cex.lab) == FALSE) stop("\"cex.lab\" is not numeric.")
        cex.lab <- 13 * cex.lab
    } else {
        cex.lab <- 13
    }
    if (is.null(cex.axis) == FALSE) {
        if (is.numeric(cex.axis) == FALSE) stop("\"cex.axis\" is not numeric.")
        cex.axis <- 12 * cex.axis
    } else {
        cex.axis <- 12
    }
    if (is.null(cex.legend) == FALSE) {
        if (is.numeric(cex.legend) == FALSE) stop("\"cex.legend\" is not numeric.")
        cex.legend <- 12 * cex.legend
    } else {
        cex.legend <- 12
    }
    if (is.null(cex.text) == FALSE) {
        if (is.numeric(cex.text) == FALSE) stop("\"cex.text\" is not numeric.")
        cex.text <- 6 * cex.text
    } else {
        cex.text <- 6
    }
    ...
    if (type %in% c("gap", "counterfactual") && sameT0 == FALSE) {
    ...
    if (type %in% c("gap", "counterfactual")) {
        ...
        time.label <- time[show]
        T.b <- 1:length(show)
        hist.adjust <- (max(time[show]) - min(time[show])) / (nT - 1)
    }
    ...
    if (type == "gap") {
        ...
        if (count == TRUE) {
            p <- plot_histogram_layer(p, data, hist.adjust, rect.min, rect.length,
                                      Ntr, max.count.pos, cex.text, type_col = NULL)
        }
        ...
        if (sameT0 == TRUE) {
            ...
            if (trim == TRUE & raw %in% c("band", "all")) {
                Nco <- sum(1 - (cumsum(sort(w.co, decreasing = TRUE)) > trim.wtot)) + 1
        ...
        if (raw == "none") {
            ...
            if (count == TRUE) {
                p <- plot_histogram_layer(p, data, hist.adjust, rect.min, rect.length,
                                          Ntr, max.count.pos, cex.text, type_col = "type")
            }
            ...
            if (trim == FALSE) {
                ...
                co.band.label <- paste0("Heavily Weighted Controls 5-95% Quantiles (",
                                        floor(trim.wtot * 100), "% weights)")
            }
        ...
        p <- p + scale_colour_manual(limits = set.limits,
            ...
            scale_size_manual(limits = set.limits,
               labels = set.labels, values = set.linewidth)
        p <- p + guides(
            linetype = guide_legend(title = NULL, ncol = legend.ncol, bycol = TRUE),
        ...
        if (stat == "mean") {
            ...
            diff <- c((bal$sd.co.pre - bal$sd.tr) / bal$sd.tr,
                      (bal$sd.co.pst - bal$sd.tr) / bal$sd.tr)
    ...
    p <- plot_apply_theme(p,
        cex.legend = cex.legend, legend.pos = legend.pos,
        cex.lab = cex.lab, cex.axis = cex.axis, cex.main = cex.main,
        angle = angle, x.h = x.h, x.v = x.v)
